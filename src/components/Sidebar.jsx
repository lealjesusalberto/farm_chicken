...
export function Sidebar({ isOpen, onClose, userData, balance, eggBalance, chickens, onLogout }) {
  ...
  chickens.forEach(chicken => {
    ...
    const eggsPerDay = (24 / eggTimeHours) * passiveMultiplier;
    
    totalEggsPerDay += eggsPerDay;
    dailyIncome += eggsPerDay * type.incomePerEgg;
  });

  const dummyHistory = [
    { type: 'recharge', amount: 50, date: '15 Ago 2026', status: 'Completado' },
    { type: 'withdraw', amount: 12, date: '12 Ago 2026', status: 'Procesando' },
    { type: 'recharge', amount: 20, date: '01 Ago 2026', status: 'Completado' }
  ];
      ...
      <div style={{
        ...
        <div style={{ padding: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.2)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            ...
            />
            <div>
              <h3 style={{ margin: 0, fontSize: '1.1rem' }}>Mi Perfil</h3>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <section>
            ...
            <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1rem', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div>
                <span style={{ color: '#ccc', fontSize: '0.8rem', display: 'block', marginBottom: '0.2rem' }}>Monedas Est.</span>
                <span style={{ fontWeight: 'bold', fontSize: '1.1rem', color: '#fbbf24' }}>+{Math.floor(dailyIncome)} H/d</span>
              </div>
              <div>
                <span style={{ color: '#ccc', fontSize: '0.8rem', display: 'block', marginBottom: '0.2rem' }}>Huevos Est.</span>
                <span style={{ fontWeight: 'bold', fontSize: '1.1rem', color: '#fff' }}>{Math.floor(totalEggsPerDay)}/d</span>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              {dummyHistory.map((tx, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', borderRadius: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                    <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: tx.type === 'recharge' ? 'rgba(74, 222, 128, 0.2)' : 'rgba(244, 63, 94, 0.2)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                      <DollarSign size={16} color={tx.type === 'recharge' ? '#4ade80' : '#ff4c4c'} />
                    </div>
                    <div>
                      <div style={{ fontSize: '0.9rem', fontWeight: 'bold' }}>{tx.type === 'recharge' ? 'Recarga' : 'Retiro'}</div>
                      <div style={{ fontSize: '0.7rem', color: '#ccc' }}>{tx.date}</div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: 'bold', color: tx.type === 'recharge' ? '#4ade80' : '#ff4c4c' }}>
                      {tx.type === 'recharge' ? '+' : '-'}${tx.amount.toFixed(2)}
                    </div>
                    <div style={{ fontSize: '0.65rem', color: tx.status === 'Completado' ? '#4ade80' : '#fcd535' }}>{tx.status}</div>
                  </div>
                </div>
              ))}
